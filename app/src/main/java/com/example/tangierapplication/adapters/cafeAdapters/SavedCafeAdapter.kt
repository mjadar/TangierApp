...
class SavedCafeAdapter(
        var liste:MutableList<Cafe>
) : RecyclerView.Adapter<SavedCafeAdapter.TodoViewHolder>(){
...
//            }
            setOnClickListener {
                val bundle = Bundle().apply {
                    putParcelable("cafe",liste[position])
                }
                findNavController().navigate(
                        R.id.action_savedCafesFragment_to_cafeFragment,
                        bundle
                )
            }
        }
